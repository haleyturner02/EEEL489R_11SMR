...
void uart_write_buffer(uart_t *uart_id, const uint8_t *data, uint16_t len){
    while (len > 0){
        ...
        len--;
				for(int i = 0; i < 100; i++){} 							// Small delay between sending characters
    }
}
...
void uart_send(uart_t *uart_id, const uint8_t *data, uint16_t len, UART_OP_CFG op){
    ...
    uart_env->tx_index = 0;																			// Set starting index at position 0
    uart_env->tx_total_length = len;														// Set to transmit length of data

    if (op == UART_OP_BLOCKING){															 	// Blocking transmit procedure
        uart_write_buffer(uart_id, data, len);									// Save starting address of data in environment
    } else if (op == UART_OP_INTR) {														// Interrupt transmit procedure
        uart_env->tx_buffer = data;
        
        NVIC_DisableIRQ(UART_INTR(uart_id));										// Disable NVIC interrupts

        uart_txdata_intr_setf(uart_id, UART_BIT_EN);						// Enable UART transmit interrupts
        uart_thre_intr_setf(uart_id, UART_BIT_EN);

        NVIC_SetPriority(UART_INTR(uart_id), uart_env->intr_priority);	// Enable interrupt priority
        NVIC_EnableIRQ(UART_INTR(uart_id));															// Enable NVIC interrupts
    }
#if defined (CFG_UART_DMA_SUPPORT)
    else
    {
